...
app.post("/api/matches/:matchId/users/:userId", async (req, res) => {
  const matchUser = await matchUsersRepository.connect(
    req.params.matchId,
    req.params.userId
  );
...
app.get("/api/matches/:matchId/states", async (req, res) => {
  const matchStates = await matchStateRepository.getByMatchId(
    req.params.matchId
  );
  const matchState =
    !matchStates || matchStates.length == 0 ? null : matchStates[0];
  res.send(matchState);
});
...
class MatchStateMachine {
  ...
  async _joinPlayer(matchState, userId, data) {
    ...
    if (!matchState.players) matchState.players = {};
