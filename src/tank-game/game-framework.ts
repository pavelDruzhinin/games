...
export class ClashPhysicEvent implements BasePhysicEvent {
  ...
  fire(scene: Scene) {
    ...
    for (let toObject of this.toObjects) {
      ...
      if (
        (fromPositionY + fromHeight >= toPositionY - toHeight ||
          fromPositionY + fromWidth >= toPositionY - toWidth) &&
        (fromPositionY - fromHeight <= toPositionY + toHeight ||
          fromPositionY - fromWidth <= toPositionY + toWidth) &&
        (fromPositionX - fromHeight <= toPositionX + toHeight ||
          fromPositionX - fromWidth <= toPositionX + toWidth) &&
        (fromPositionX + fromHeight >= toPositionX - toHeight ||
          fromPositionX + fromWidth >= toPositionX - toWidth)
      ) {
        ...
        if (
          damageObject &&
          !damageObject.damage.isLastDamage(this.fromObject.damage)
        ) {
...
export class StrikingDistancePhysicEvent implements BasePhysicEvent {
  ...
  fire(scene: Scene) {
    ...
    if (this._object.positionY <= this._strikingPosition) {
      if (this._animation) {
        this._animation.setPosition(
          this._object.positionX,
          this._strikingPosition
        );
...
export class Scene {
  ...
  private _randomTimer: number;
  private _randomDrawObjects: List<BaseDrawObject> = new List<BaseDrawObject>();
  ...
  constructor(canvasId: string, width: number, height: number) {
    let scene: HTMLCanvasElement = <HTMLCanvasElement>(
  ...
  addRandomDrawObjects(randomDrawObjects: BaseDrawObject[]) {
    for (const randomDrawObject of randomDrawObjects) {
