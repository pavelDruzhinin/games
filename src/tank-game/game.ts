...
class TankPanelAmmunition {
  ...
  constructor(ammunition: TankAmunnition) {
    this._panel = document.getElementsByClassName(
      "tank-ammunition-panel-inner"
    )[0];
...
class TankGame {
  ...
  start(state?: MatchState) {
    ...
    let position =
      state == null
        ...
        : state.players[GameStorage.instance.userId].position;
    ...
    function tankFire() {
      ...
      for (var bullet of bullets) {
        ...
        const animation =
          typeof shrapnel.createStrikeAnimation == "function"
            ? shrapnel.createStrikeAnimation()
            : null;
        ...
        if (shrapnel.strikingDistance && animation) {
          let event = new StrikingDistancePhysicEvent(
            bullet,
            bullet.positionY,
            shrapnel.strikingDistance,
            animation
          );
          game.scene.addPhysicEvent(event);
        }
  ...
  _addEnemyWithoutDraw(
    enemyId: number,
    startPositionX: number,
    startPositionY: number,
    direction: TankDirections
  ) {
  ...
  addEnemy(
    ...
    const enemyTank = this._addEnemyWithoutDraw(
      enemyId,
      startPositionX,
      startPositionY,
      direction
...
client.addSocketListener(GameEventType.JoinPlayer, (gameData: GameData) => {
  if (gameData.userId == GameStorage.instance.userId) return;

  tankGame.addEnemy(
    gameData.userId,
    gameData.data.positionX,
    gameData.data.positionY,
    gameData.data.direction
  );
...
function joinMatch(matchId: number) {
  ...
  const randomPositionX =
    Math.random() *
    (tankGame.game.scene.width - 50) *
    tankGame.game.scene.devicePixelRatio;
  const randomPositionY =
    Math.random() *
    (tankGame.game.scene.height - 50) *
    tankGame.game.scene.devicePixelRatio;
...
document.getElementById("add-match").addEventListener("click", (event) => {
