...
export class Box extends BaseDrawObject {
    ...
    constructor(strategy: BaseBoxStrategy, sceneWidth: number, sceneHeight: number) {
        super();
        ...
        this._image = new GameImage(`/assets/img/${BoxTypeImage[strategy.imageType]}.png`);
    }
...
export class BoxFactory {
    static create(sceneWidth: number, sceneHeight: number): List<Box> {
        ...
        for (let i = 0; i < boxLength; i++) {
            const boxTypeIndex = randomRange(0, boxTypes.length);
