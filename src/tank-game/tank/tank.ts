import { BaseDrawObject, Colors, GameStorage } from "../game-framework";
...
export class Tank extends BaseDrawObject implements IDamagable {
  ...
  move(direction: TankDirections) {
    ...
    client.sendGameData(
      new GameData(this._storage.userId, GameEventType.ChangePosition, {
  ...
  _turn(direction: TankDirections) {
    ...
    var currentDirectionScope = this.getCurrentDirectionScope(
      directions,
      currentDirectionIndex
    );
    console.log(currentDirectionScope);

    var currentDirectionScopeIndex = currentDirectionScope.indexOf(
